...
##librerias para customizar ----
#install.packages("gcookbook") #para las etiquetas
#library(gcookbook)
#library("ggplot2")

##numero de proyectos por tipo ---- 
...
bd_reactiva_pyto %>%
  ...
  ungroup() %>%
  ggplot() +
  aes(x=TIPOLOGIA, fill=TIPOLOGIA) +
  geom_bar() +
  geom_text(aes(label=after_stat(count)), stat = "count", vjust=-0.2)


#(etiqueta dentro: vjust=1.5,  colour= "white")

#ordenado, 1 solo color,  etiquetas.
bd_reactiva_pyto %>%
  group_by(TIPOLOGIA) %>%
  summarise(count=n()) %>%
  ...
  geom_text(aes(label=after_stat(y)), vjust=-0.2) +
  theme(legend.title = element_blank()) +
  labs(x=" ", y=" ", title = "GRAFICO 1. NÚMERO DE PROYECTOS POR CLASE")



#ordenado, colores,  etiquetas y titulo.
bd_reactiva_pyto %>%
  group_by(TIPOLOGIA) %>%
  summarise(count=n()) %>%
  ggplot(aes(x=reorder(TIPOLOGIA, (-count)), y=count, fill=TIPOLOGIA)) +
  geom_col(colour="black") +
  geom_bar(stat = 'identity') +
  geom_text(aes(label=after_stat(y)), vjust=-0.2)+
  labs(x=" ", y=" ", title = "GRAFICO XX. NÚMERO DE PROYECTOS POR CLASE") +
...
#ordenado, colores,  etiquetas y titulo (falta darle formato a los soles)
bd_reactiva_pyto %>%
  group_by(TIPOLOGIA) %>%
